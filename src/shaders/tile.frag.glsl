...
const float QUANTIZED_COLOR_COUNT = 4.f;
...
vec2 random2To2(vec2 in_v) {
  // PCG2D hash taken from https://www.shadertoy.com/view/XlGcRh
  uvec2 v = uvec2(ivec2(in_v)) * 1664525u + 1013904223u;
  v.x += v.y * 1664525u;
  v.y += v.x * 1664525u;
  v = v ^ (v >> 16u);
  v.x += v.y * 1664525u;
  ...
  // Multiply by (1 / UINT_MAX) to get a value between 0 and 1
  return vec2(v) * (1.f / 4294967295.f);
}
...
float quantize(float value) {
  return floor(value * (QUANTIZED_COLOR_COUNT - 1.0f) + 0.5f) / (QUANTIZED_COLOR_COUNT - 1.0f);
}
...
float ordered_dither(float value) {
  ...
  float final = value + (threshold - 0.5f) / (QUANTIZED_COLOR_COUNT - 1.f);
...
void main() {
  ...
  vec3 video_dither = vec3(ordered_dither(video_color.r), ordered_dither(video_color.g), ordered_dither(video_color.b));

  float worley_dither = ordered_dither(worley_value);
  vec3 worley_color = mix(u_color_a, u_color_b, worley_dither);

  // Apply easing ramp from from https://easings.net/#easeInOutQuart
  if (worley_dither < 0.5f) {
    worley_dither = 8.f * worley_dither * worley_dither * worley_dither * worley_dither;
  } else {
    worley_dither = 1.f - pow(-2.f * worley_dither + 2.f, 4.f) * 0.5f;
  }

  // Clamp it between 0.7 to 1.0 so the dithered video can't get too dark
  worley_dither = clamp((worley_dither * 0.3f) + 0.7f, 0.f, 1.f);

  vec3 mixed_color = mix(worley_color, video_dither * worley_dither, u_mix);
