...
blurb: RCW is an endless minigame about matching the right colors and words together. The longer you survive, the higher your score! Play it now on itch.io.
tags:
  - game
  - graphics
...
In Random Color Words (RCW) you are constantly trying to pair the correct combination of colors and words together. Both of these are randomly generated at the start of each round. Incredibly strong innovative start here, I know.

I saw this project as a way to involve myself with every part of a game development cycle. This led to me getting my hands dirty with things other than programming, like the audio or the UI. And being new to... well, everything, I focused my attention on simple but _polished_ and _fleshed out_ results. And hopefully, a little fun to play.

I think I achieved that goal. I'm proud to be able to publish RCW as a "complete package."

## Brainstorming gameplay

To begin, I started brainstorming simple gameplay ideas that didn't take too much time to implement. This means that the gameplay loop should be simple, and the best way to achieve that is to make the game _infinite_ and have no ending. Just like an arcade game.
...
<figure>
  <VideoGif content={{ collection: "posts", folder: "rcw" }} name="oldtv" />
  <figcaption>**OLDTV.** In-game footage. It's free on Steam, go play it.</figcaption>
</figure>

But uh, you're not exactly pressing next on the TV remote. What is happening?

### Explanation

Essentially, each round we're given a color word, and a word color—uh, what? Actually, ignore the game for a second—let's just consider these two words below.
...
This is the premise on which OLDTV works. Given the current word and color combination, left click if the color matches the word, and right click if it doesn't. To make your life even more fun, OLDTV can (and will) flip your screen upside down, change the background color, and even reverse the controls. Oh, and there's a time limit so you're forced to react quickly.
...
- I also have a score system that depends on how fast you react. More on that below.
...
<figure class="mx-auto flex max-w-[275px] items-end gap-5">
...
From what I've read some disadvantages include having to modify/keep global state and increased inflexibility. To be fair, people from the links above aren't arguing against using it at all, but to consider whether it's suitable for the current context; it seems like it's misused/overused often.
...
C# is incredible. I discovered their existence in the middle of development, and started converting many things over, then back, but not everything; so the game runs in a sorta singleton/observer hybrid. Again, code's a bit messy.
...
Events especially came in handy

### Other stuff

- coroutines
- c# properties, structs, and classes

## Music and SFX

I know I said I wanted to dabble in a little bit of everything, but I wasn't going to sit there and _start learning music production_, so this time I turned to the internet for some help.
