...
The site now has a light color scheme and supports light mode. Officially.[^4] This took some reconfiguring of CSS variables and discovering hardcoded color styles, and in general took longer than I'd like to admit.

I learned recently you can style selected text and media via the `::selection` pseudo-element, so I've do<span class="bg-sweater-8 text-sweater-2 not-dark:bg-sweater-2 not-dark:text-sweater-8">ne just that like you can see here. Not e</span>verything [is fully supported on all browsers](https://developer.mozilla.org/en-US/docs/Web/CSS/::selection#browser_compatibility) but this is purely visual and should gracefully fall back to defaults.
...
[^4]: My `<meta name="color-scheme">` always claimed it did, which I'm sure threw off some user agents. My bad.
