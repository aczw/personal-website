...
First I had to connect with Astro Studio and also add the Astro DB integration. Both of these were relatively simply to set up.

This project only has two routes: a main `index.astro` for the root, and a `[...slug].astro` file that captures everything else. The former is for creating new links, and the latter is where we do the redirecting.

## Creating short links

> Why is every short link exactly 7 characters? Because "Spotify" is 7 characters and I wanted a custom link with the slug `spotify` that redirects to my profile. It's why I started this whole thing in the first place!

## Redirecting

Astro has a built-in way of modifying the response returned by the server. In particular, I set the status code to 301 and set the `Location` header to the original URL. Inside `[...slug].astro`:
...
One interesting thing here is that I wrote no HTML for the `/[...slug]` route. The only code in `[...slug].astro` is in the Astro frontmatter, which only gets executed on the server. If no short URL exists in the database, then I simply redirect to the home page.
