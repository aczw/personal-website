...
All potential redirects are handled in `[...slug].astro` inside the frontmatter. We first check if the slug in the current URL exists in the database. I select all the rows where the short link matches the slug (and there should only be one slug), and from that row I extract the original URL.
...
Choosing colors is hard, so I used [Randoma11y](https://randoma11y.com/) to generate an accessible color palette for me. I wanted to keep things simple, so the whole site is just two colors and one font, and I picked [Commit Mono](https://commitmono.com/) because it's a nice neutral and readable monospaced font.
...
> [video showing the button width changing or something]
