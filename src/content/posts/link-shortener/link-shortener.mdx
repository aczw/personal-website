...
The backend receives a `POST` request from the form, with the form data as the body. I use [`nanoid`](https://go.czw.sh/D9rxQRQ) which generates a random string of characters for me. I'm using an alphanumeric alphabet with a size of 7.[^1]
...
I'm not sure if you've noticed, but every single external URL I linked to in this post has been ran through the link shortener :)
...
[^1]: Why 7? Because "Spotify" is 7 characters, the entire point of this endeavor! Could I increase it for security and longevity? Of course, but then, no Spotify :(
