...
> As of February 2026, the backend has stopped working, and I don't really feel like fixing it, so... you had a good run! RIP 💔💔💔
...
To get around that, I use link shorteners, but I've found that they're very unreliable, and often shut down/stop working. With the recent release of Astro Studio and [Astro DB](https://astro.build/db), I thought this was the perfect chance to test it out... and hopefully resolve my woes once and for all.[^1]

The source code for this project [is available on GitHub](https://github.com/aczw/link-shortener) and to try it out yourself, visit [go.czw.sh](https://go.czw.sh).

## Setting up the project

First I had to connect with Astro Studio and also add the Astro DB integration. Both of these were relatively simple to set up. Studio integration is explained well, and examples for [interacting with the DB](https://docs.astro.build/en/guides/astro-db) are even provided for people that don't know SQL (me).

One thing that wasn't too clear was where I should create the `config.ts` file. It turns out that it should be placed in a `db` folder outside of your `src` folder.[^2] Otherwise, local type definitions weren't being generated for me in the `.astro` folder.
...
The backend receives a `POST` request from the form, with the form data as the body. I use [`nanoid`](https://github.com/ai/nanoid) which generates a random string of characters for me. I'm using an alphanumeric alphabet with a size of 7.[^3]
...
- search engines and crawlers should never show the short URL. `308` also seems to be a valid alternative to `301`, but from what I learned it mostly provides more consistency with non-`GET` operations. We only use `GET`, so `301` works fine.
...
Choosing colors is hard, so I used [RandomA11y](https://randoma11y.com) to generate an accessible color palette for me. I wanted to keep things simple, so the whole site is just two colors and one font, and I picked [Commit Mono](https://commitmono.com) because it's a nice and neutral monospace font.
...
> **10/1/24 update:** [Astro Studio is shutting down](https://astro.build/blog/goodbye-astro-studio). I am very upset. I will take a look at Turso, which is what the docs recommend as a replacement. They do seem to have a free plan and there is [a migration guide](https://docs.astro.build/en/guides/astro-db/#migrate-from-astro-studio-to-turso), but... _sigh_.
...
[^1]: If you think the premise is a bit silly, I promise you it's not; a simple search for ["spotify link shortener"](https://duckduckgo.com/?q=spotify+link+shortener) returns a nontrivial number of _dedicated websites_ for doing this. People like sharing music, what can I say?

[^2]: Installing Astro DB with the `astro add` command seems to take care of this for you automatically, but I did it manually.

[^3]: Why 7? Because "Spotify" is 7 characters, the entire point of this endeavor! Should I increase it for security and longevity? Of course, but then, no perfect `/spotify` link :(
