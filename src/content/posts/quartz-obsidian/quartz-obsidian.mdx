---
title: Setting up Quartz with Obsidian
blurb: Exposing my unfinished notes online for fun and (no) profit
posted: 2025-01-08T20:51:00-04

slug: quartz-obsidian
---

Live at [otherworld.czw.sh](https://otherworld.czw.sh). Yes, the name is [once again](/projects/door) inspired by OMORI.
...
Quartz turns your local Obsidian vault into a fully-fledged, functional website. Compared to Obsidian Publish, it’s free and you get to customize many more things. This post documents my experience with Quartz and how I got it to work with my vault.

> Up to date as of v4.4.0.
...
> I like Obsidian Sync, but unfortunately I am a Broke College Student. Also, this method has worked great thus far and I see no reason to switch.
...
const config: QuartzConfig = {
  /* ...other stuff... */
  plugins: {
    transformers: [Plugin.CrawlLinks({ markdownLinkResolution: "shortest" })],
  },
};
```
...
```ts title="quartz.config.ts" ins={5}
const config: QuartzConfig = {
  /* ...other stuff... */
  plugins: {
    transformers: [
...
<div class="mx-pad rounded-lg border-dashed border-line-dark px-0 not-dark:border-(length:--border-w)">
  ![Homepage of the my Quartz site, displayed in light purple
  colors.](quartz-light.png)
</div>
...
> I finished and exported before Illustrator could even finish booting up. #ProfessionalHater

### Bun, Windows, and Quartz commands

Honestly, I’m not sure what’s going on here, and who is at fault.[^4] But Quartz does not work with [Bun](https://bun.sh) out of the box. In particular, while Quartz’s documentation uses Node and `npm`, analogous commands like `bunx quartz sync` would fail.

```
> bunx quartz sync
error: could not determine executable to run for package quartz
```
...
bun: command not found: ./quartz/bootstrap-cli.mjs
error: script "quartz" exited with code 1
```

Oops. Looking at the error, this one’s on me. It states we’re trying to _literally_ run the file, which unfortunately doesn’t work on Windows because it doesn’t recognize the shebang present at the top of this file. The solution was to prepend `bun run` to the script so that Windows knows which executable to use.

```json title="package.json" ins={4} del={5}
{
  /* ...etc... */
  "scripts": {
    "quartz": "bun run ./quartz/bootstrap-cli.mjs",
...
> I’ve tried to avoid rearranging my vault to fit Quartz; I view the relationship the other way around, with Quartz being a sole consumer of my vault’s contents.
>
> Originally I thought about writing some sort of custom logic that chooses to copy over a file only if it’s referenced at least once in one of the notes, but this folder thing was just simpler to do.

Here is the entirety of my workflow file:

```yaml title=".github/workflows/update-content.yml"
...
> If you’re wondering, this is also why I chose the GitHub Actions route instead of including my entire vault repo as a Git submodule. It was simply easier to write a copying script that runs as part of the job, especially when I only need files from like two folders.
>
> FYI I tried out submodules at first, but as of this writing [Vercel doesn't really support private submodules](https://github.com/vercel/community/discussions/44) without some extra finagling.
