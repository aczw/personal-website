...
- _Finally_ fix interop between Prettier and Astro. I don't have to manually run the formatter anymore! The VS Code extension formats on save now.
- Make headings use the monospace font by default. Declared in the `@layer` directive.
- `/projects` now uses a two-column setup. This really lets the images and blurbs stand out more.
...
So what I did was try adapting a little snippet from [this Astro guide](https://docs.astro.build/en/tutorial/6-islands/2/#add-client-side-interactivity), which checks the browser's local storage before setting the theme.
