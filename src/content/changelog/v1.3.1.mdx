...
Also, I just want to point out that this is not that easy to do with React, which expects components to render the same way on every rerender (given the same inputs). I'm not even sure if this would pass React's strict mode.
