...
For the final project of our computer graphics class, we created a 3D voxel engine basically from scratch. We were clearly influenced by Minecraft, but as someone who grew up playing it, I took the chance to change some things I've always wished looked a bit different.
...
I'll talk a bit about each, some implementation details, and any other interesting details. Let's get started.
...
> Why not a 3D array? A flattened array enables faster lookup times due to the way it is laid out in memory.
...
> `glm::ivec2` comes from [OpenGL Mathematics](https://github.com/g-truc/glm), a commonly-used library that implements vectors, matrices and other utilities in an OpenGL-friendly manner.
...
> I did the base algorithm implementation, and it works well. My teammate decided to jazz it up and add colored lights! That's what you see in the final video.
