...
<div class="grid grid-cols-1 gap-4 xs:grid-cols-2">
  <figure>![Zero metallicness, a little roughness](sphere-no-metallic.png)</figure>
  <figure>![Full metallicness, more roughness](sphere-full-metallic.png)</figure>
</div>
...
<VideoGif content={{ collection: "projects", folder: "pbr-renderer" }} name="model-ibl" />

As you can see, we're getting pretty good performance! My laptop has a 120 Hz screen and the renderer is dropping no frames (the recording is limited to 24 FPS unfortunately).
...
<div class="grid grid-cols-1 gap-4 xs:grid-cols-2">
  <figure>![Zero displacement on black obsidian block](obsd-none.png)</figure>
  <figure>![Full displacement of obsidian block.](obsd-some.png)</figure>
</div>
