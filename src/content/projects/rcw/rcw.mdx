---
name: RCW
subtitle: Endless arcade-like Unity minigame
blurb: My first experience with game dev, RCW is a simple game about matching the right colors and words together.
metadata:
  tech:
    - Unity
    - C#
...
In Random Color Words (RCW)[^1] you are constantly trying to pair the correct combination of colors and words together. Both of these are randomly generated at the start of each round.
...
<figure>
  ...
  />
  <figcaption>
    **OLDTV.** In-game footage. It's free on Steam, go play it.
  </figcaption>
</figure>
...
{
    Audio.Instance.sfxSource.PlayOneShot(
      AudioClips.Instance.buttonHover
    );
}
```
...
var offset = new Vector2(speed * _x, speed * _y);

cb.uvRect = new Rect(
    cb.uvRect.position + offset * Time.deltaTime,
...
[^1]: Incredibly creative name, I know.
