...
<Img
  src={import("./face-culling.png")}
  alt="Two blocks are right next to each other. The face in between does not need to be rendered."
/>
...
<Img
  src={import("./hollowed-caves.png")}
  alt="Since we're not rendering any blocks inside the terrain, we can see snake-like caves underground."
/>
...
I render chunks using <Link href="https://www.opengl-tutorial.org/intermediate-tutorials/tutorial-9-vbo-indexing/" newTab>indexed rendering</Link> and store all vertex attributes in a single, <Link href="https://www.khronos.org/opengl/wiki/Vertex_Specification#Interleaved_attributes" newTab>interleaved</Link> VBO. This includes UV texture information and a single "animated?" flag that I either set to 0 or 1.
...
The <Link href="https://registry.khronos.org/OpenGL-Refpages/gl4/html/glVertexAttribPointer.xhtml" newTab>"stride" parameter</Link> in `glVertexAttribPointer` was difficult to wrap my head around. All it does is tell the GPU when the data for the next vertex starts. To the GPU, our VBO is just a really, really long line of floats. So, we must also specify the size of each attribute.

<Img
  src={import("./interleaved-vbo.png")}
  alt="In a stream of floats, we specify the first four to be for position, the next four to be for normals, and so on."
/>
...
<Img
  src={import("./block-uv.png")}
  alt="A dirt face texture at coordinate (2, 1) corresponds to (2/16, 1/16) in UV coordinates."
/>
...
<Img
  src={import("./distance-fog.png")}
  alt="Blocks slowly fade into the distance in a circular radius around the player."
/>
...
<Img
  src={import("./distance-fog-diagram.png")}
  alt="Diagram depicting the fog starting roughly around 96 blocks around the player in all directions."
/>
...
<Img
  src={import("./distance-fog-pretty.png")}
  alt="A beautiful sunrise over the desert."
/>
...
<Img
  src={import("./day-night-cycle.png")}
  alt="A diagram depicting what percentage of a day we spend in day, in night, and how long it takes to transition."
/>
...
<Img
  src={import("./skybox-uv.png")}
  alt="The sky is split into green, red, and black."
/>
...
In our <Link href="https://doc.qt.io/qt-6/qopenglwidget.html#paintGL" newTab>`paintGL()`</Link>, after drawing the sun and moon and before drawing the terrain, I reset the OpenGL depth buffer. This means that they are _always_ drawn behind blocks, no matter the player's y-coordinate.
...
<Img
  src={import("./clouds-ps.png")}
  alt="Cloud texture is opened in Photoshop."
/>
...
This way, I can offset the UV coordinates based on the player's current world position. When the player moves forward, the UV coordinates translate "back," which gives the clouds the appearance of having been left behind. Take a look at <Link href="https://github.com/jdah/minecraft-weekend/blob/387ecb554946180094d58c57a4ee216dba506cc4/src/world/sky.c#L350C17-L350C17" newTab>jdh's implementation</Link> to get an idea of the math.
...
- <Link
    href="https://www.reddit.com/r/gamedev/comments/2iru8i/fast_flood_fill_lighting_in_a_blocky_voxel_game/"
    newTab
  >
    This Reddit post
  </Link>
- <Link
    href="https://0fps.net/2018/02/21/voxel-lighting/"
    newTab
  >
    0 FPS - Voxel lighting
  </Link>
- <Link
    href="https://minecraft.wiki/w/Light#Block_light"
    newTab
  >
    Minecraft Wiki
  </Link>
- <Link
    ...
    newTab
  >
    Wikipedia article about flood fill
  </Link>
...
<Img
  src={import("./ui-scaling.png")}
  alt="Diagram showing how the UI is scaled up and doesn't matter what screen resolution we are."
/>
...
<Img
  src={import("./font-ps.png")}
  alt="The font bitmap file is open in Photoshop."
/>

The idea is simple: map a basic set of characters to UV coordinates on a texture sprite that contains the characters as a bitmap. This is not a new idea (<Link href="https://en.wikipedia.org/wiki/Computer_font#Bitmap_fonts" newTab>far, far from it</Link>) and there are probably thousands of implementations available online, but
...
No! There must exist a better way. And there does. Remember that in C++, the `char` type is really just <Link href="https://en.cppreference.com/w/cpp/language/ascii" newTab>ASCII representations</Link> of the characters. If we order the bitmap characters in ASCII order, generating the map on the fly becomes really easy:
...
<Img
  src={import("./debug-mode.png")}
  alt="Debug mode is turned on, and shows the current player position as (32, 158, 32) and chunk coordinate as (32, 32)."
/>
