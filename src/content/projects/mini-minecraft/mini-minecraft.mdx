...
We're allowed to define our UI element positions on a relative 0.0-1.0 scale because the `UI` class takes care of transforming it into actual positions based on our current working screen resolution. Furthermore, the x-values are offset by a margin that means everything stays centered.
...
The hotbar is rendered the same way as the crosshair. The little overlay added on top of the currently active slot is rendered by adding an additional offset to the x-position depending on what slot number we're on.
