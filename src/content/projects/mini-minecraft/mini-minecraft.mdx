...
In our <Link href="https://doc.qt.io/qt-6/qopenglwidget.html#paintGL" newTab>`paintGL()`</Link>, after drawing the sun and moon and before drawing the terrain, I reset the OpenGL depth buffer. This means that they are *always* drawn behind blocks, no matter the player's y-coordinate.
...
Remember `VertexAttrs` from earlier? We add a new attribute called `lightLevel`. In our fragment shader, we multiply our default block color by this additional brightness. Make sure to clamp it between 0.0-1.0 so the block doesn't get *too* bright.
...
<Img src={import("./font-ps.png")} alt="The font bitmap file is open in Photoshop." />
...
<Img src={import("./debug-mode.png")} alt="Debug mode is turned on, and shows the current player position as (32, 158, 32) and chunk coordinate as (32, 32)." />
...
Overall, this project has been very rewarding and is probably the most fun I've had with a class project, ever. I've learned a lot about OpenGL, C++, and rendering in general. I've also become *very* aware of all the heavy lifting that engines like Unity or Unreal do for us.
